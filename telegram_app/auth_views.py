...
def telegram_auth_code(request):
    ...
    if request.method == 'POST':
        ...
        if not code:
            ...
            return render(request, 'telegram_auth_code.html')
...
def set_password_view(request):
    """
    Yangi username va parol o'rnatish (birinchi marta kirganlar uchun)
    """
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if User.objects.filter(username=new_username).exclude(id=user_id).exists():
                messages.error(request, f"'{new_username}' login allaqachon band! Boshqa login tanlang.")
                return render(request, 'set_password.html')
            ...
            messages.success(request, f"Xush kelibsiz, {user.first_name or user.username}! Login va parol muvaffaqiyatli o'rnatildi.")
            return redirect('dashboard')
...
def forgot_password_view(request):
    """
    Parolni unutganlar uchun - Telegram orqali tiklash
    """
    if request.method == 'POST':
        ...
        try:
            ...
            messages.info(request, f"Login topildi: {username}. Endi Telegram orqali tasdiqlash.")
            return redirect('telegram_auth_phone')
            
        except User.DoesNotExist:
            messages.error(request, "Bunday login nomi topilmadi!")
            return render(request, 'forgot_password.html')
